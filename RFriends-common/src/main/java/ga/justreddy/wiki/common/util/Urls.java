...
public class Urls {
  ...
  public static void download(Dependency dependency, File folder, BiConsumer<File, File> whenDone) {
    ...
    try {
      ...
      DependencyLoader.log(Level.SEVERE,
              "[RFriends] Failed to download the dependency " + dependency.getName());
    }
  }


  private static void tryDownload(String fileUrl, File file, String... customUrl) throws Exception {
    DependencyLoader.debug("Attempting to download " + fileUrl);
    ...
    for (String url : REPOSITORIES) {
      ...
      DependencyLoader.debug("URL is '" + actualUrl + "'");
      try {
        openStream(actualUrl, (fUrl, stream) -> pullFromStreamToFile(stream, fUrl, file));
        return;
      } catch (IOException e) {
        DependencyLoader.log(Level.WARNING, "Failed to download from repo '" + url + "'");
      }
    }

    DependencyLoader.log(Level.SEVERE, "Failed to download " + fileUrl);
  }
  ...
  private static void pullFromStreamToFile(InputStream stream, String url, File file) {
    try {
      ...
      if (!file.getName().endsWith(".jar") || !DependencyLoader.isEnforcingFileCheck()) return;
      ...
      openStream(url + ".sha1", (shaUrl, shaStream) -> {
        ...
        try {
          ...
          DependencyLoader.debug("Maven SHA-1: " + mavenSha1, "File SHA-1: " + fileSha1);
          ...
          DependencyLoader.debug("File " + file.getName() + " passed validation");
      ...
      DependencyLoader.log(Level.SEVERE, "[RFriends] Failed to download URL to FILE " + file.getName());
    }
  }
