package ga.justreddy.wiki.rfriendscommon.spigot.util;
...
public class Urls {
    ...
    public static void download(Dependency dependency, File folder, BiConsumer<File, File> whenDone) {
        ...
        try {
            ...
            e.printStackTrace();
            DLoaderSpigot.log(Level.SEVERE, "Failed to download dependency " + dependency.getName());
        }
    }


    private static void tryDownload( String fileUrl,  File file,  String... customUrl) throws Exception {
        DLoaderSpigot.debug("Attempting to download " + fileUrl);
        ...
        for (String url : REPOSITORIES) {
            ...
            DLoaderSpigot.debug("URL is '" + actualUrl + "'");
            try {
                openStream(actualUrl, (fUrl, stream) -> pullFromStreamToFile(stream, fUrl, file));
                return;
            } catch (IOException e) {
                DLoaderSpigot.log(Level.WARNING, "Failed to download from repo '" + url + "'");
            }
        }

        DLoaderSpigot.log(Level.SEVERE, "Failed to download " + fileUrl);
    }
    ...
    private static void pullFromStreamToFile( InputStream stream, String url,  File file) {
        try {
            ...
            if (!file.getName().endsWith(".jar") || !DLoaderSpigot.isEnforcingFileCheck()) return;
            ...
            openStream(url + ".sha1", (shaUrl, shaStream) -> {
                ...
                try {
                    ...
                    DLoaderSpigot.debug("Maven SHA-1: " + mavenSha1, "File SHA-1: " + fileSha1);
                    ...
                    DLoaderSpigot.debug("File " + file.getName() + " passed validation");
            ...
            e.printStackTrace();
            DLoaderSpigot.log(Level.SEVERE, "Failed to download url to file " + file.getName());
        }
    }
